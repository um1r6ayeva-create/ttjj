// i18n/dutyPage.ts
export const dutyPage = {
  ru: {
    title: 'Система дежурств',
    adminSubtitle: 'Панель администратора - назначение дежурств',
    commandantSubtitle: 'Панель коменданта - подтверждение дежурств',
    studentSubtitle: 'Мои дежурства - отправка отчетов',
    globalDuty: 'Общие дежурство',
    regularDuty: 'Обычные дежурства',
    roles: {
      admin: 'администратор',
      user: 'студент',
      commandant: 'комендант',
      student: 'студент',
      elder: 'староста'
    },
    renderError: 'Произошла ошибка при отображении панели. Пожалуйста, обновите страницу.'
  },
  en: {
    title: 'Duty System',
    adminSubtitle: 'Admin Panel - Duty Assignment',
    commandantSubtitle: 'Commandant Panel - Duty Confirmation',
    studentSubtitle: 'My Duties - Report Submission',
    globalDuty: 'Community Duty',
    regularDuty: 'Regular Duties',
    roles: {
      admin: 'administrator',
      user: 'student',
      commandant: 'commandant',
      student: 'student',
      elder: 'elder'
    },
    renderError: 'An error occurred while rendering the panel. Please refresh the page.'
  },
  uz: {
    title: 'Navbatchilik tizimi',
    adminSubtitle: 'Administrator paneli - navbatchilik belgilash',
    commandantSubtitle: 'Komendant paneli - navbatchilikni tasdiqlash',
    studentSubtitle: 'Mening navbatchiliklarim - hisobot topshirish',
    globalDuty: 'Umumiy navbatchilik',
    regularDuty: 'Oddiy navbatchiliklar',
    roles: {
      admin: 'administrator',
      user: 'talaba',
      commandant: 'komendant',
      student: 'talaba',
      elder: 'boshliq'
    },
    renderError: 'Panelni ko\'rsatishda xatolik yuz berdi. Iltimos, sahifani yangilang.'
  }
};